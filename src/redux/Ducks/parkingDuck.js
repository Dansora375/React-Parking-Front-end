...
import {IngresoResidente} from  '../../services/entrada.service'
...
// Llenar parking
const FILL_PARKING_RESI_SUCCESS='FILL_PARKING_RESI_SUCCESS'
const FILL_PARKING_RESI_ERROR='FILL_PARKING_RESI_ERROR'
...
export default function parkingReducer(state =initialParkings, action){
  ...
  switch(action.type){
    ...
    case CRAETE_NEW_PARKING_SUCCESS:
      ...
      case FILL_PARKING_RESI_SUCCESS:
        return {...state, parkingsResi:action.payload }
      case FILL_PARKING_RESI_ERROR:
        return {...state.error, parkingsResi:action.payload.message}
    default:
      return state
  }
}
  ...
  try {
    ...
    if (res.completed) {
      ...
      if(res.data.data.personType==='Residente'){
        const newState=getState().Parkings.parkingsResi.slice(0)
        ...
        const newState=getState().Parkings.parkingsVisi.slice(0)
        newState.push(res.data.data) 
         // console.log(getState())
        // console.log(newState)
        dispatch({
...
export const FillParkingAction=(info)=>async(dispatch, getState)=>{
  try {
    const res = await IngresoResidente(info)
    if (res.completed){
      const updateState=getState().Parkings.parkingsResi.slice(0)
      const dataIndex= updateState.map(item=>item._id).indexOf(info.IdParking)
      updateState.splice(dataIndex ,1 ,res.data.data.resultParking)
      dispatch({
        type:FILL_PARKING_RESI_SUCCESS, 
        payload:updateState
      })
    ...
    dispatch({
      type:FILL_PARKING_RESI_ERROR, 
      payload:{message:`ha ocurrido un error al obtener el
      parquedero de visitante: ${error}` }
  })
  }
}
